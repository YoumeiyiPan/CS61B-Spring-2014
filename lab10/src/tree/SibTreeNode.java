...
//	        	SibTreeNode kid = firstChild;
	        	if(N==0) {   
	            	...
	            	((SibTreeNode)(this.child(N))).nextSibling=node;
	            } else if(N!=0 && c<=N){
...
*/
  ...
  public void removeLeaf() throws InvalidNodeException {
    // sFILL IN YOUR SOLUTION TO PART III HERE.
	  
	    if (isValidNode()) {
	    	if (this.parent!=null){
	    	 SibTreeNode node=this.parent;
	    	int N=this.children();
	        if (N!=0) {
	          
	        } else {
	        	TreeNode cur=node.child(1);
	        	if (cur==this) {
	        	...
	        	for (int i=1;i<=node.children();i++){
	        		...
	        		if (node.child(i+1)==this){
	        			((SibTreeNode)node.child(i)).nextSibling=((SibTreeNode)node.child(i+1)).nextSibling;
	        			((SibTreeNode)node.child(i+1)).parent=null;
	        			((SibTreeNode)node.child(i+1)).valid=false;
	        			myTree.size--;
	        		}
	        	}
	        	}
	      ...
	      } else if(this.parent==null&&this.children()==0){
	    	  this.parent=null;
	    	  this.valid=false;
	    	  myTree.size--;
	      }
